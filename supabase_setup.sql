...
-- 3. V2 learning state (thinking, learner SQL, evaluation and practice stages)
-- Existing installations can instead run migrations/002_learning_progress.sql.
...
create or replace function public.merge_learning_progress(incoming jsonb, expected_user uuid)
returns jsonb
language plpgsql
security invoker
...
begin
  ...
  insert into public.learning_progress (user_id) values (actor)
  on conflict (user_id) do nothing;
  select state into stored from public.learning_progress where user_id = actor for update;
  stored := coalesce(stored, '{"version":2,"resetAt":0,"entries":{}}'::jsonb);
  merged_reset := greatest(coalesce((stored ->> 'resetAt')::bigint, 0), coalesce((incoming ->> 'resetAt')::bigint, 0));
  ...
  loop
    ...
    if greatest(existing_updated, incoming_updated) <= merged_reset then continue; end if;
  ...
  merged_state := jsonb_build_object('version', 2, 'resetAt', merged_reset, 'entries', merged_entries);
  update public.learning_progress set state = merged_state, updated_at = now() where user_id = actor;
  return merged_state;
end;
$$;
